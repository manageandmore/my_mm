 ...
 */
export function getPostCreatorModal(options: PostCreatorModalOptions): ModalView {
...
export function getPostImageUrl(options: PostCreatorModalOptions, encode: boolean): string {
  return `https://${currentUrl}/api/social${makeQueryParams(options, encode)}`
}

function makeQueryParams(params: Record<string, string | number | undefined>, encode: boolean): string {
  if (encode) {
    const data = Object.fromEntries(Object.entries(params).filter(([_, v]) => v != null))
