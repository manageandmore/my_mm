 ...
 */
export async function getUserIdFromScholarId(scholarId: string): Promise<string> {
...
async function getEmailForScholar(scholarId: string): Promise<string | null> {
  try {
    const response = await notion.pages.retrieve({page_id: scholarId}) as ScholarRow
