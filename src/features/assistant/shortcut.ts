...
slack.messageShortcut(addToAssistantShortcut, async (request) => {
  ...
  const documentId = await toUUID(payload.channel.id + ':' + payload.message_ts)
  ...
  title = title.replaceAll('<!here>', '@here').replaceAll('<!channel>', '@channel')
...
async function toUUID(id: string): Promise<string> {
  let digest = await crypto.subtle.digest('SHA-1', new TextEncoder().encode(id))
